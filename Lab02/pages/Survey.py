...
with st.form("survey_form"):
    ...
    if submitted:       
        ...
        if category_input and value_input:
            ...
            new_data = pd.DataFrame([{'category': category_input, 'value': value_input}])

            # Check if the file exists to determine if a header is needed
            if not os.path.exists('data.csv'):
                # Write with headers if the file is new
            ...
            st.warning("Please fill in both the category and value fields.") 
